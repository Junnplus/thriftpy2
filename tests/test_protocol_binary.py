...
def test_pack_i8():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.I08, 123)
    assert "7b" == hexlify(b.getvalue())


def test_unpack_i8():
    b = BytesIO(b"{")
    assert 123 == TBinaryProtocol(b).read_val(TType.I08)


def test_pack_i16():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.I16, 12345)
    assert "30 39" == hexlify(b.getvalue())


def test_unpack_i16():
    b = BytesIO(b"09")
    assert 12345 == TBinaryProtocol(b).read_val(TType.I16)


def test_pack_i32():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.I32, 1234567890)
    assert "49 96 02 d2" == hexlify(b.getvalue())


def test_unpack_i32():
    b = BytesIO(b"I\x96\x02\xd2")
    assert 1234567890 == TBinaryProtocol(b).read_val(TType.I32)


def test_pack_i64():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.I64, 1234567890123456789)
    assert "11 22 10 f4 7d e9 81 15" == hexlify(b.getvalue())


def test_unpack_i64():
    b = BytesIO(b"\x11\"\x10\xf4}\xe9\x81\x15")
    assert 1234567890123456789 == TBinaryProtocol(b).read_val(TType.I64)


def test_pack_double():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.DOUBLE, 1234567890.1234567890)
    assert "41 d2 65 80 b4 87 e6 b7" == hexlify(b.getvalue())


def test_unpack_double():
    b = BytesIO(b"A\xd2e\x80\xb4\x87\xe6\xb7")
    assert 1234567890.1234567890 == TBinaryProtocol(b).read_val(TType.DOUBLE)


def test_pack_string():
    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.STRING, "hello world!")
    assert "00 00 00 0c 68 65 6c 6c 6f 20 77 6f 72 6c 64 21" == \
        hexlify(b.getvalue())

    b = BytesIO()
    TBinaryProtocol(b).write_val(TType.STRING, u("你好世界"))
...
def test_unpack_string():
    ...
    assert u("你好世界") == TBinaryProtocol(b).read_val(TType.STRING)


def test_unpack_binary():
    bs = BytesIO(b"\x00\x00\x00\x0c"
                 b"\xe4\xbd\xa0\xe5\xa5\xbd\xe4\xb8\x96\xe7\x95\x8c")
    assert u("你好世界").encode("utf-8") == TBinaryProtocol(
        bs, decode_response=False).read_val(TType.STRING)


def test_write_message_begin():
    b = BytesIO()
    TBinaryProtocol(b).write_message_begin("test", TType.STRING, 1)
...
def test_write_message_begin_not_strict():
    b = BytesIO()
    TBinaryProtocol(b, strict_write=False) \
...
def test_write_struct():
    ...
    TBinaryProtocol(b).write_struct(item)
...
def test_read_struct():
    ...
    _item2 = TItem()
    TBinaryProtocol(b).read_struct(_item2)
    assert _item == _item2
